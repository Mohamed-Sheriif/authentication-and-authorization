...
app.get("/api/auth/2fa/generate", ensureAuthenticated, async (req, res) => {
  try {
    ...
    const uri = authenticator.keyuri(user.email, "mohamed-sherif", secret);
...
app.get(
  ...
  (req, res) => {
    res.redirect("http://localhost:3000/api/users/current");
  }
);

app.get("/api/auth/logoutAll", ensureAuthenticated, async (req, res) => {
  try {
